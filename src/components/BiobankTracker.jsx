...
export default function BiobankTracker({ shipments, setShipments, requests, setRequests, loggedInLabId }) {
  // Filter shipments to only show those originating from this biobank
  const labShipments = shipments.filter((s) => s.labId === loggedInLabId);
  
  const [selectedShipmentId, setSelectedShipmentId] = useState(labShipments[0]?.id || null);
  const [viewMode, setViewMode] = useState('table');
  const [sortField, setSortField] = useState('id');
  const [sortDirection, setSortDirection] = useState('asc');

  const selectedShipment = labShipments.find((s) => s.id === selectedShipmentId);
  ...
  const advanceShipment = (shipmentId) => {
    ...
    // If fully delivered, update request status to 'Delivered'
  ...
  const sortedShipments = [...labShipments].sort((a, b) => {
    ...
    <div className="tracker-layout">
      ...
      <div className="glass-panel">
        <div className="panel-header">
          <h2>
            <Truck size={18} style={{ color: 'var(--accent-teal)' }} /> Node Dispatch Controls
          </h2>
          <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
            ...
            <span className="badge badge-progress" style={{ fontSize: '11px' }}>
              {labShipments.length} Dispatched
            </span>
          </div>
        </div>

        <div className="panel-body" style={{ maxHeight: '70vh', overflowY: 'auto' }}>
          {labShipments.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '40px', color: 'var(--text-muted)' }}>
              No shipments currently registered for your node inventory. Fulfill a paid request in the LIMS dashboard to start tracking.
            </div>
          ) : viewMode === 'table' ? (
              ...
              return (
                ...
                >
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px', color: 'var(--text-secondary)', marginTop: '6px' }}>
                    <span>Sample ID: {shipment.sampleId}</span>
                  </div>
      ...
      {selectedShipment ? (
            ...
            {selectedShipment.currentStepIndex < selectedShipment.steps.length - 1 ? (
              <div style={{ 
                ...
                <div>
                  ...
                  <span style={{ fontSize: '11px', color: 'var(--text-secondary)' }}>Advance shipment to next checkpoint to simulate transit logistics.</span>
                </div>
        ...
        <div className="glass-panel" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '400px', color: 'var(--text-muted)' }}>
          Select a shipment to manage transit simulation checkpoints.
        </div>
      )}
    </div>
  );
